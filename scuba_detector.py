...
import sys
import numpy as np
import subprocess # Tambahan untuk meluncurkan pemutar video eksternal

# --- KONFIGURASI ---
VIDEO_PATH = "scuba.mp4"
REQUIRED_MOTION_FRAMES = 5 
MOUTH_COVER_SENSITIVITY = 12 # Sensitivitas cahaya diturunkan ke batas wajar
MOUTH_TEXTURE_DROP = 4       # Ambang batas hilangnya tekstur (saat bibir ditutup kulit tangan yang rata)
MOTION_SENSITIVITY = 5       # Sensitivitas gerakan lambaian tangan

# --- PENGATURAN IMPORT AUDIO (Sebagai Fallback) ---
HAS_AUDIO = False
try:
...
def play_scuba_video():
    ...
    print("\n[INFO] Memutar video SCUBA...")
    
    # ---------------------------------------------------------
    # SOLUSI AUDIO: Menggunakan Video Player OS (Lebih Stabil)
    # ---------------------------------------------------------
    try:
        if sys.platform.startswith('linux'):
            # Memutar dengan 'mpv' dalam mode Window (Bukan Fullscreen)
